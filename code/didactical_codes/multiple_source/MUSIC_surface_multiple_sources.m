...
angles = zeros(P,iterations);
kk = 1;
for snrValue = snr(1):1:snr(end)
    ...
    Pmusic_end = 10*log10(Pmusic/max(Pmusic)); % Determinando pico
    [Max,angles(:,ite)] = findpeaks(Pmusic_end,theta,'SortStr','descend','NPeaks',P);

    end
    
    error = doa - mean((sort(angles)/180*pi)');
    RMSE(kk) = sqrt( mean( mean(error).^2 ) );
    MSE(kk) = mean( mean(error).^2 );
