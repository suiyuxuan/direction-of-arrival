...
for gsnr_i = gsnr_min:gsnr_step:gsnr_max
    ...
    %error_as(k) = immse(tau_as, delay*ones(1,n_iter));
    %error_as_nlt(k) = immse(tau_as_nlt, delay*ones(1,n_iter));
    %error_as_mod(k) = immse(tau_mod, delay*ones(1,n_iter));
end

plot(snr_min:snr_step:snr_max,error_awgn,'b')
hold on
plot(gsnr_min:gsnr_step:gsnr_max,error_as,'r')
plot(gsnr_min:gsnr_step:gsnr_max,error_as_nlt,'k')
plot(gsnr_min:gsnr_step:gsnr_max,error_as_mod,'g')
